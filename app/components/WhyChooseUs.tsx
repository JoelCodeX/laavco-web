...
export default function WhyChooseUs() {
  ...
  const getIcon = (iconName: string) => {
    switch (iconName) {
      case "award":
        return (
          <svg className="w-5 h-5 sm:w-6 sm:h-6 text-brand-orange" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
  ...
  return (
    <section id="nosotros" className="w-full bg-white py-20 px-6 md:px-12 lg:px-24">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="lg:col-span-7 grid grid-cols-2 sm:grid-cols-2 gap-3.5 sm:gap-6">
          {whyChooseUsSection.items.map((pilar, index) => (
            <div
              key={pilar.id}
              className={`bg-blue-50 p-3.5 sm:p-6 rounded-xl border border-blue-100 flex flex-col gap-2.5 sm:gap-3 transition-all duration-300 hover:shadow-md hover:border-brand-orange/20 ${
                index === whyChooseUsSection.items.length - 1 ? "col-span-2 sm:col-span-2" : ""
              }`}
            >
              <div className="w-9 h-9 sm:w-12 sm:h-12 bg-white rounded-lg flex items-center justify-center shadow-sm border border-neutral-100 shrink-0">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-[10px] sm:text-sm text-neutral-600 leading-relaxed font-medium">
                  {pilar.description}
                </p>
              </div>
            </div>
